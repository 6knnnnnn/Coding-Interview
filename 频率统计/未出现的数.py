...
# Can you write code to calculate the number of missing integers between these two #indices.

# For example nums = [2,4,7,8,9,15],
# of missing integer between 0,1 is 1, nums[0] = 2,nums[1] = 4, 2, 3, 4, -> 3, as 1 number
...
def find_missing_numbers(nums, i, j):
    ...
    # iterate all number z in [x, y], if z is not in nums[i...j], z is a missing number, then we increase the total count
    ...
    # 0,2 we have 2,4,7, we need iterate [3, 6], then check if a number is in set(4), 3,4,5,6, O(N) time in total
...
def find_nth_missing_number(nums, n):
    ...
    # space O(1), time O(N), N = the number of elements in nums, N-1, each time call lib, O(1), in total = O(N)
    # binary search, pre-process the nums to get another array, with the total number of misssing in nums
    ...
    # two ranges, [i...k] [k...j], then call the lib to get missing number of each range, left and right,
